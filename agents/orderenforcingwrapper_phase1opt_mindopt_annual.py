...
from agents.user_agent_phase1opt_mindopt_annual import UserAgent
...
class OrderEnforcingAgent:
    ...
    def register_reset(self, observation):
        ...
        self.building_info = observation['building_info']
        for agent_id in range(self.num_buildings):
            action_space = self.action_space[agent_id]
            build_info = self.building_info[agent_id]
            self.agent.set_action_space(agent_id, action_space)
            self.agent.set_building_info(agent_id, build_info)

        # initialize optimization model
        self.agent.init_opt_model(obs)
