...
if __name__ == "__main__":
    ...
    targets = [item for item in y_train.columns if 'Solar_Generation_' in item]

    start_time = time.time()
    print("------------------train---------------------------")
    gbm = MultiOutputRegressor(
        lgb.LGBMRegressor(objective='regression', n_estimators=5000, learning_rate=0.05, num_leaves=32, random_state=2022,
                          verbose=-1, n_jobs=1), n_jobs=24)
    gbm.fit(x_train, y_train)
    print('Training time: {}'.format(time.time() - start_time))

    # Save to file in the current working directory
    pkl_filename = "agents/checkpoints/lgb_solar_multi_speedup_new_1030_heavy.pkl"
